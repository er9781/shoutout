  ...
  console.log(
    [
      ...
      ...[
        ...
        `You may also pass a macro pattern. Macro patterns define blocks of size ${MACRO_SIZE} which alternate 
...
// repeat passedPatterns to fill second if only 1 passed :D, then drop the rest since only 2 supported
...
// Create a tokenFinder which is parametrized per pattern differently. A bit hacky.
const parts = partition(list, nocenter ? 2 : 3);
const tokenFinders = _.fromPairs(
  passedPatterns.map((passedPattern, idx) => {
    const [
      first = DEFAULT_FIRST,
      second = DEFAULT_SECOND,
      center = first
...
for (r of Array(totRows).keys()) {
  ...
  for (c of Array(totCols).keys()) {
    const pattern = macroFinders[macro](
      ...
      Math.floor(r / MACRO_SIZE),
      Math.floor(c / MACRO_SIZE)
    );
